<x-dashboard-layout>
            ...
            <div class="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
                <div>
                    <h2 class="text-3xl sm:text-4xl font-black text-white tracking-tight mb-2">
                        Edit <span class="text-transparent bg-clip-text bg-gradient-to-r from-[#E6B65C] to-[#CCA050]">Buku</span>
                    </h2>
                    <p class="text-gray-300 text-lg max-w-xl">
                        Update informasi, perbaiki typo, atau ganti file buku kamu.
                    </p>
                </div>
    ...
    <div class="max-w-4xl mx-auto">
        <div class="rounded-2xl border border-gray-100 bg-white shadow-xl p-8">
            ...
            <form method="POST" action="{{ route('publisher.books.update', $book) }}" enctype="multipart/form-data" class="space-y-6">
                @csrf
                @method('PUT')

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">Judul Buku</label>
                    <input name="title" class="w-full rounded-xl border-gray-200 focus:border-[#E6B65C] focus:ring focus:ring-[#E6B65C]/20 transition" value="{{ old('title', $book->title) }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input name="genre" class="w-full rounded-xl border-gray-200 focus:border-[#E6B65C] focus:ring focus:ring-[#E6B65C]/20 transition" value="{{ old('genre', $book->genre) }}" placeholder="Contoh: Fiksi, Teknologi, Bisnis">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <input name="price" type="number" class="w-full pl-12 rounded-xl border-gray-200 focus:border-[#E6B65C] focus:ring focus:ring-[#E6B65C]/20 transition" value="{{ old('price', $book->price) }}">
                        </div>
                ...
                <div>
                    ...
                    <textarea name="description" class="w-full rounded-xl border-gray-200 focus:border-[#E6B65C] focus:ring focus:ring-[#E6B65C]/20 transition" rows="5">{{ old('description', $book->description) }}</textarea>
                ...
                <div>
                    ...
                    <div class="relative">
                         <select name="status" class="w-full appearance-none rounded-xl border-gray-200 focus:border-[#E6B65C] focus:ring focus:ring-[#E6B65C]/20 transition px-4 py-3 bg-white">
                            <option value="draft" @selected(old('status', $book->status)=='draft')>Draft (Disimpan dulu)</option>
                            <option value="published" @selected(old('status', $book->status)=='published')>Published (Langsung tayang)</option>
                            <option value="unpublished" @selected(old('status', $book->status)=='unpublished')>Unpublished (Tarik dari katalog)</option>
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div>
                        ...
                        @if($book->cover_path)
                            <div class="mb-3 flex items-center gap-3 p-3 rounded-lg border border-gray-100 bg-gray-50">
                                <img src="{{ asset('storage/'.$book->cover_path) }}" class="h-16 w-12 object-cover rounded shadow" alt="">
                                <div class="text-xs text-gray-500">Cover saat ini</div>
                            </div>
                        @endif
                        ...
                        <p class="text-xs text-gray-400 mt-2">Upload hanya jika ingin mengganti cover.</p>
                        @error('cover') <div class="text-red-600 text-sm mt-1 font-medium">{{ $message }}</div> @enderror
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-2">File E-Book (Opsional)</label>
                         <div class="relative group">
                        ...
                        <p class="text-xs text-gray-400 mt-2">Upload hanya jika ingin mengganti file PDF.</p>
